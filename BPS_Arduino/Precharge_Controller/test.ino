...
void precharge() {
  ...
  */
  ...
  if (digitalRead(Optocoupler) == LOW) { //if Optocoupler is active
    ...
    if (charginsgInterval > prechargeInterval) {
      carRunning = true;
...
void precharge_fault() {
  ...
  Serial.println(BMS_DischargeEn == LOW);
  while(1);
}
...
void loop() {
  ...
  Serial.println(carRunning);
  if (carRunning == false) {
      if (digitalRead(BMS_DischargeEn) == LOW ) { //&& digitalRead(BMS) == HIGH
        if (prechargeStart == MAX_TIMER) {   //if the prechargeStart hasn't yet been assigned         
            ...
            Serial.println("Pre-Charge AIR on");
            prechargeStart = millis();
            }
        Serial.println("Start Pre-Charge");
        precharge();
      }
  }
